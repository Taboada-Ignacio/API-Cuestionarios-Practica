...
export default function AcademicPicker({label,value,entries,onSelect,disabled,loading,specialOptions=true,required=true}){
 const id=useId(),root=useRef(null),[query,setQuery]=useState(''),[open,setOpen]=useState(false),[active,setActive]=useState(-1);
 const availableSpecial=specialOptions?special:[];
 const selected=[...availableSpecial,...entries].find(e=>e.id===value);
 ...
 const options=[...matches.slice(0,60),...availableSpecial];
 ...
 return <div className="academic-field academic-picker" ref={root} onBlur={e=>{if(!e.currentTarget.contains(e.relatedTarget)){setOpen(false);setActive(-1);}}}>
  ...
  <small id={id+'-help'} className="picker-help">{disabled?'Primero seleccioná la opción anterior, si corresponde.':value?'Selección confirmada. Podés escribir para cambiarla.':specialOptions?'Seleccioná un resultado o elegí «No posee».':'Seleccioná un resultado del catálogo.'}</small>
 </div>;
}
